...
# NETWORK
TEST_NETWORK='hoteldemo_network'

# DB
TEST_DB_DATA='hoteldemo_test_db_data'
TEST_DB_NAME='hoteldemo_test_db'
TEST_DB_IMAGE='mysql'
TEST_DB_PASSWORD='password'

TEST_REDIS_SERVER_NAME='hoteldemo_test_redis'
...
isDBUp() {
  x=1
  while [[ $(docker logs $TEST_DB_NAME --tail 100 2>/dev/null) != *"Ready for start up"* ]] && [ ${x} -le 30 ]; do
    sleep 1
    x=$((x + 1))
    printf "."
  done

  if [[ $(docker logs $TEST_DB_NAME --tail 100 2>/dev/null) != *"Ready for start up"* ]]; then
