...
isDBUp() {
  x=1
  logs=$(docker container logs --tail 100 hoteldemo_mysql 2>/dev/null)
  # shellcheck disable=SC2039
  while [[ $logs != *"started."* ]] && [ ${x} -le 19 ]; do
    sleep 1
    x=$((x + 1))
    printf "."
    logs=$(docker container logs --tail 100 hoteldemo_mysql 2>/dev/null 2>&1)
  done
  ...
  if [[ $logs != *"started."* ]]; then
    ...
    printf "\nLOGS:"
    docker container logs --tail 100 hoteldemo_mysql 2>/dev/null 2>&1
    exit 1
  else
